...
def generate_xls(user_id, create_at_start, create_at_end,
                 ...
                 trade_state, trade_type, channel, total_num):
    '''创建时间 商户交易号 平台交易号 商户名称 所属渠道商 支付类型 交易金额 当前状态'''
    wb = Workbook()
    inlet_info_ws = wb.create_sheet(u'商户交易信息', 0)
    # fields = [u'商户编号'.encode('GB18030'), u'创建时间'.encode('GB18030'),
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, create_at_start, create_at_end,
                                        out_trade_no, mch_trade_no, wx_transaction_id,
...
def db_inlet_info(user_id, create_at_start, create_at_end,
                  out_trade_no, mch_trade_no, wx_transaction_id,
                  trade_state, trade_type, channel):
    data = []
...
def deal_data(data):
    for _d in data:
        ...
        if _d[5]:
            _d[5] = u'JS支付' if _d[7] == 'alipay' and str(_d[5]) == 'JSAPI' else WX_TRADE_TYPE[str(_d[5])]
        if _d[6]:
            _d[6] = _d[6] / 100
        if _d[7]:
...
def gen_inlet_info(ws, datas):
    fields = [u'商户编号', u'创建时间', u'平台交易号', u'商户交易号',
...
def general_filename(user_id):
    _ts = str(time.time())
    filename = "mch_inlet_trade_export_" + _ts + ".xlsx"
