...
@app.task
def generate_xls(user_id, dt_name, dt_id, create_at_start, order_by, total_num):
    wb = Workbook()
    ...
    try:
        ...
        dist_statistics_data = db_statistics_info(dt_name, dt_id, create_at_start, order_by)
...
def gen_order_download_info(order_id, user_id, file_name):
    ...
    db.executeSQL(query, (order_id, user_id, file_name, 10, 1, 2, create_at, update_at))
...
def db_statistics_info(dt_name, dt_id, create_at_start, order_by):
    ...
    _query = """SELECT dt_user.dt_name, dt_user.dt_id,
                 ...
                 WHERE (dtb.pay_start_time between %(create_at_start)s::timestamp and %(create_at_end)s::timestamp)
