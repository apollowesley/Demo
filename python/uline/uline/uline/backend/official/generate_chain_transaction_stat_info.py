...
@app.task
def generate_xls(chain_id, create_at_start, create_at_end):
    ...
    inlet_info_data = db_transaction_stats(
        chain_id, create_at_start_search, create_at_end_search)
...
def db_transaction_stats(chain_id, create_at_start, create_at_end):
    query = """select
                  to_char(ci.need_pay_time - INTERVAL '1 day', 'YYYY-MM-DD'),
                   ci.day_tx_count,
                   ci.day_tx_amount,
                   ci.day_refund_count,
                   abs(ci.day_refund_amount),
                   ci.day_tx_net_amout,
                   abs(ci.day_profit_amount)
                   ...
                   and (ci.need_pay_time between %(create_at_start)s::timestamp and %(create_at_end)s::timestamp);"""
