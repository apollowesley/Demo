...
def db_inlet_info(dt_name, mch_name, dt_id, mch_id, complete_at_start, complete_at_end,
    ...
    if ul_mch_id:
        ...
        # 需要用到in查询
        if mch_name or dt_name or mch_id or dt_id or cs_mch_id or cs_mch_shortname:
            tb_query = """select
                    ul_mch_id,
...
def gen_inlet_info(cw):
    fields = [u'商户编号', u'支付时间', u'商户交易号', u'平台交易号',
              u'当前状态', u'支付类型', u'交易金额', u'支付渠道', u'备注', u'商户名称', u'所属渠道商', u'商户简称']
    cw.writerow(fields)
    # 商户交易信息
    # ws.append(fields)
    # for data in datas:
