...
@app.task
def generate_xls(mch_id, create_at_start, create_at_end, query_date):
    ...
    inlet_info_data = db_transaction_stats(
        mch_id, create_at_start_search, create_at_end_search, query_date)
    if int(query_date) == 2:
    ...
    file_path, static_path = general_filename(mch_id)

    wb.save(file_path)
    return static_path


def db_transaction_stats(mch_id, create_at_start, create_at_end, query_date):
    query = """select
                    {switch}
                sum(mch.day_tx_count),
                round(sum(mch.day_tx_amount), 2),
                sum(mch.day_refund_count),
                abs(round(sum(mch.day_refund_amount),2)),
                round(sum(mch.day_tx_net_amout),2),
                round(sum(mch.day_profit_amount),2)
                from mch_daily_balance_info AS mch
                left join mch_user on mch.mch_id = mch_user.mch_id
    ...
    switch = " to_char(mch.need_pay_time - INTERVAL '1 day', 'YYYY-MM-DD') as pay_time," if int(
        query_date) == 1 else "to_char(mch.need_pay_time - INTERVAL '1 day','YYYY-MM') as pay_time,"
    date_query = "mch.need_pay_time" if int(
        query_date) == 1 else "mch.need_pay_time - INTERVAL '1 day'"
...
def general_filename(mch_id):
    _ts = str(time.time())
    filename = "transaction_statistic_export_" + _ts + ".xlsx"
    user_dl_path = path.join(cur_dir, "static/downloads/", str(mch_id))
    if not path.exists(user_dl_path):
        makedirs(user_dl_path)
    file_path = path.join(user_dl_path, filename)
    static_path = path.join("/static/downloads/", str(mch_id), filename)
