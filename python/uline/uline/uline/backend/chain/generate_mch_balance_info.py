...
@app.task
def generate_xls(user_id, mch_name, mch_id, create_at_start, create_at_end, pay_status, total_num, channel, dt_sub_id):
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, mch_name, mch_id,
                                        create_at_start, create_at_end, pay_status, channel, dt_sub_id)
...
def db_inlet_info(user_id, mch_name, mch_id, create_at_start, create_at_end, pay_status, channel, dt_sub_id):
    query = """select
           ...
           inner join dt_user as du on du.dt_id = mii.cs_id
           left join dt_subuser as dsu on mii.dt_sub_id = dsu.dt_sub_id
           where du.dt_id=%(dt_id)s
           ...
           and (mii.mch_name ~ %(mch_name)s or %(mch_name)s is null)
           and (mii.mch_id::VARCHAR ~ %(mch_id)s::VARCHAR or %(mch_id)s::VARCHAR is null)
    ...
    if ret:
        ...
        for index in range(0, len(ret)):
            ret[index][6], ret[index][7], ret[index][4], ret[index][8], ret[index][11] = long(ret[index][6]) / 100, TO_PAY[str(ret[index][7])],\
                PAY_CHANNEL[ret[index][4]], str(
                    ret[index][8]), str(ret[index][11])
        return ret
    return ret


def gen_inlet_info(ws, datas):
    fields = [u'结算日期', u'门店名称', u'所属渠道商', u'结算户名', u'支付渠道', u'结算账号',
              u'金额', u'状态', u'门店结算单号', u'门店编号', u'门店简称', u'结算银行']
