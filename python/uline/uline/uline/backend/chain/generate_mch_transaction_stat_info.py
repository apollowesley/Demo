...
@app.task
def generate_xls(mch_id, create_at_start, create_at_end):
    '''日期 支付笔数 支付成功金额 退款笔数 退款金额 交易总净额 手续费'''
    wb = Workbook()
    inlet_info_ws = wb.create_sheet(u'交易数据统计', 0)
    create_at_end_search = create_at_end + timedelta(1)
    create_at_start_search = create_at_start + timedelta(1)
    inlet_info_data = db_transaction_stats(
        mch_id, create_at_start_search, create_at_end_search)
...
def db_transaction_stats(mch_id, create_at_start, create_at_end):
    query = """select
                to_char(mch.need_pay_time - INTERVAL '1 day', 'YYYY-MM-DD'),
                mch.day_tx_count,
                mch.day_tx_amount,
                mch.day_refund_count,
                abs(mch.day_refund_amount),
                mch.day_tx_net_amout,
                abs(mch.day_profit_amount)
                ...
                and (mch.need_pay_time between %(create_at_start)s::timestamp and %(create_at_end)s::timestamp);"""
