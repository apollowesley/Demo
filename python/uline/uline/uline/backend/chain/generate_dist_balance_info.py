...
def db_inlet_info(user_id, create_at_start, create_at_end, pay_status, channel):
    ...
    if ret:
        ...
        for index in range(0, len(ret)):
            ret[index][6], ret[index][5], ret[index][3], ret[index][7], ret[index][8] = TO_PAY[str(ret[index][6])],\
                ret[index][5] / 100, PAY_CHANNEL[ret[index][3]
                                                 ], str(ret[index][7]), str(ret[index][8])
        return ret
    return ret


def gen_inlet_info(ws, datas):
    fields = [u'结算日期', u'所属渠道商', u'结算户名', u'支付渠道',
