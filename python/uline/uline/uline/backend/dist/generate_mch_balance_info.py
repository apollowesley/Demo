...
@app.task
def generate_xls(user_id, mch_name, mch_id, create_at_start, create_at_end, pay_status, total_num, channel, dt_sub_id, cs_mch_short_name, cs_mch_id):
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, mch_name, mch_id,
                                        create_at_start, create_at_end, pay_status, channel, dt_sub_id, cs_mch_short_name, cs_mch_id)
...
def db_inlet_info(user_id, mch_name, mch_id, create_at_start, create_at_end, pay_status, channel, dt_sub_id, cs_mch_short_name, cs_mch_id):
    query = """select
    ...
    query_dict = {
        ...
        'cs_mch_short_name': db.escape(cs_mch_short_name)

    }

    query_dict.update({'cs_mch_short_name': db.escape(cs_mch_short_name),
                       'mch_id': db.escape(cs_mch_id)}) if cs_mch_short_name or cs_mch_id else \
...
def gen_inlet_info(ws, datas):
    fields = [u'结算日期', u'交易日期', u'所属渠道商', u'商户编号', u'商户名称', u'商户简称', u'门店编号', u'门店名称', u'门店简称', u'结算银行', u'支行联行号', u'结算户名',
              u'结算账号', u'支付渠道',  u'金额', u'状态', u'备注', u'业务员编号', u'业务员名称']
