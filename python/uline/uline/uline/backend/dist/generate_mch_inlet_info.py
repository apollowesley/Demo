...
# 生成商户进件信息
@app.task
def generate_xls(user_id, mch_name, mch_id, create_at_start, create_at_end, activated_status, auth_status, total_num, dt_sub_id):
    wb = Workbook()
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, mch_name, mch_id, create_at_start,
                                        create_at_end, activated_status, auth_status, dt_sub_id)
...
def db_inlet_info(user_id, mch_name, mch_id, create_at_start, create_at_end, activated_status, auth_status, dt_sub_id):
    query = """
            SELECT s.* from (
                SELECT
...
def gen_inlet_info(ws, datas, industry_name):
    fields = [u'商户编号', u'商户简称', u'商户名称', u'微信商户编号', u'省份', u'城市', u'联系地址', u'联系人', u'联系电话',
              u'电子邮箱', u'结算方式', u'结算账号类型', u'结算户名', u'支行名称', u'支行联行号', u'结算账号', u'身份证号码', u'审核状态',
              u'是否使用渠道商微信appid', u'业务员编号', u'业务员姓名', u'行业类别']
    # 渠道基本信息表
    ws.append(fields)
    ...
    for data in datas:
        data = list(data)
        data[10], data[11], data[17], data[18] = BALANCE_WAY[str(data[10])], BALANCE_TYPE[str(data[11])], AUTH_STATUS[
            str(data[17])], WX_USE_PARENT[str(data[18])]
