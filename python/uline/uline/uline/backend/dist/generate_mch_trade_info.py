...
@app.task
def generate_xls(user_id, mch_name, mch_id, complete_at_start, complete_at_end,
                 out_trade_no, mch_trade_no, wx_transaction_id,
                 trade_state, trade_type, channel, total_num, dt_sub_id, cs_mch_shortname, cs_mch_id):
    ...
    fields = [u'支付时间'.encode('GB18030'), u'所属渠道商'.encode('GB18030'),
              ...
              u'门店简称'.encode('GB18030'), u'业务员id'.encode('GB18030'),
              u'业务员名称'.encode('GB18030'), u'商户交易号'.encode('GB18030'),
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, mch_name, mch_id, complete_at_start, complete_at_end,
                                        out_trade_no, mch_trade_no, wx_transaction_id,
                                        trade_state, trade_type, channel, dt_sub_id, cs_mch_shortname, cs_mch_id)
...
def db_inlet_info(user_id, mch_name, mch_id, complete_at_start, complete_at_end,
                  out_trade_no, mch_trade_no, wx_transaction_id,
                  trade_state, trade_type, channel, dt_sub_id, cs_mch_shortname, cs_mch_id):
    data = []
    ul_query = """select
...
def gen_inlet_info(ws, datas):
    fields = [u'商户编号', u'支付时间', u'平台交易号', u'商户交易号',
              u'当前状态', u'支付状态', u'交易金额', u'支付渠道', u'备注', u'商户名称', u'所属渠道商', u'业务员编号', u'业务员名称', u'商户简称']
    # 商户交易信息
    ws.append(fields)
    for data in datas:
        ws.append(list(data))
