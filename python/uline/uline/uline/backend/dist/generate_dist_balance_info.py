...
def db_inlet_info(user_id, create_at_start, create_at_end, pay_status, channel):
    ...
    if ret:
        ...
        for index in range(0, len(ret)):
            ...
            ret[index][7], ret[index][6], ret[index][4], ret[index][8], ret[index][9] = TO_PAY[str(ret[index][7])],\
                ret[index][6] / 100, PAY_CHANNEL[ret[index][4]
                                                 ], str(ret[index][8]), str(ret[index][9])
        return ret
    return ret


def gen_inlet_info(ws, datas):
    fields = [u'结算日期', u'交易时间', u'所属渠道商', u'结算户名', u'支付渠道',
