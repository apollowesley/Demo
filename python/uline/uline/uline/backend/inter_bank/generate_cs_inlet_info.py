...
# 生成商户进件信息
@app.task
def generate_xls(user_id, chain_name, cs_name, cs_id, chain_id, create_at_start, create_at_end, activated_status,
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, chain_name, cs_name, cs_id, chain_id, create_at_start, create_at_end,
                                        activated_status, auth_status)
        id_to_name = {data[0]: data[1] for data in inlet_info_data}
        # dt_types = get_dt_type(id_to_name)
...
def db_inlet_info(user_id, chain_name, cs_name, cs_id, chain_id, create_at_start, create_at_end, activated_status, auth_status):
    query = """
    SELECT s.* from (
        SELECT
            ...
            INNER JOIN mch_user on mch_inlet_info.mch_id = mch_user.mch_id
              INNER JOIN dt_user on dt_user.dt_id = mch_inlet_info.cs_id
            ...
            WHERE (mch_inlet_info.activated_status=%(activated_status)s OR %(activated_status)s IS NULL )
              ...
              and (dt_user.dt_id = %(chain_id)s or %(chain_id)s is null)
              AND (dt_user.dt_name ~ %(chain_name)s or %(chain_name)s IS NULL  )
              AND (mch_inlet_info.mch_id::VARCHAR ~ %(mch_id)s::VARCHAR or %(mch_id)s::VARCHAR IS NULL  )
              AND (mch_inlet_info.mch_shortname ~ %(short_name)s OR %(short_name)s IS NULL )
...
def gen_inlet_info(ws, datas, industry_name):
    fields = [u'门店编号', u'门店简称', u'门店名称', u'微信商户编号', u'渠道商名称', u'省份', u'城市', u'联系地址', u'联系人', u'联系电话',
    ...
    for data in datas:
        data = list(data)
        data[11], data[12], data[18] = BALANCE_WAY[str(data[11])], BALANCE_TYPE[str(data[12])], AUTH_STATUS[
            str(data[18])]
        # if data[22] == 1:
        #     data[20], data[21] = "", ""
        # del data[22]
