...
@app.task
def generate_xls(user_id, dt_name, mch_name, dt_id, mch_id, create_at_start, create_at_end,
                 out_trade_no, mch_trade_no, transaction_id, refund_status,
                 mch_refund_no, out_refund_no, refund_id,
                 channel, total_num, cs_mch_shortname, cs_mch_id):
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, dt_name, mch_name, dt_id, mch_id, create_at_start, create_at_end,
                                        out_trade_no, mch_trade_no, transaction_id, refund_status,
                                        mch_refund_no, out_refund_no, refund_id,
                                        channel, cs_mch_shortname, cs_mch_id)
        ...
        for ul_data in ul_ret:
            if int(tb_data[0]) == ul_data[5]:
                temp = tb_data[0:2] + [ul_data[0], ul_data[1], ul_data[2], ul_data[3], ul_data[4], ul_data[5],
                                       ul_data[6], ul_data[7], ul_data[8]] + tb_data[2:]
                data.append(temp)
    return data
...
def gen_inlet_info(ws, datas):
    fields = [u'创建时间',
              u'所属渠道商', u'所属渠道商编号', u'商户编号', u'商户名称', u'商户简称', u'门店编号', u'门店名称', u'门店简称',
