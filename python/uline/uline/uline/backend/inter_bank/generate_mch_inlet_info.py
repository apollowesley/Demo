...
def db_inlet_info(user_id, dt_name, mch_name, dt_id, mch_id, create_at_start, create_at_end, activated_status, auth_status):
    query = """
    SELECT s.* from (
        SELECT
            ...
            INNER JOIN mch_user on mch_inlet_info.mch_id = mch_user.mch_id
              ...
              LEFT JOIN dt_inlet_info on dt_inlet_info.dt_id = mch_inlet_info.dt_id
...
def gen_inlet_info(ws, datas, industry_name):
    fields = [u'商户编号', u'商户简称', u'商户名称', u'微信商户编号', u'渠道商名称', u'省份', u'城市', u'联系地址', u'联系人', u'联系电话',
              ...
              u'审核状态', u'是否使用渠道商微信appid', u'业务员编号', u'业务员姓名', u'行业类别']
    # 渠道基本信息表
    ws.append(fields)
