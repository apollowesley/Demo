...
# 任务队列
@app.task
def generate_xls(user_id, dt_name, dt_id, create_at_start, create_at_end, activated_status, auth_status, total_num):
    """
    ...
    try:
        inlet_info_data = db_inlet_info(user_id, dt_name, dt_id, create_at_start,
...
def gen_order_download_info(order_id, user_id, file_name):
    ...
    db.executeSQL(query, (order_id, user_id, file_name, 2, 1, 1, create_at, update_at))
...
def db_inlet_info(user_id, dt_name, dt_id, create_at_start, create_at_end, activated_status, auth_status):
    query = """
    SELECT s.* from (
        select
        dt_inlet_info.dt_id,
...
def gen_inlet_info(ws, datas, industry_name):
    fields = [u'渠道商编号', u'渠道商名称', u'省份', u'城市', u'联系地址', u'联系人', u'联系电话', u'电子邮箱',
              u'结算方式', u'结算账号类型', u'支行名称', u'支行联行号', u'结算账号', u'身份证号码', u'审核状态',
              u'行业类别']
    # 渠道基本信息表
    ws.append(fields)
