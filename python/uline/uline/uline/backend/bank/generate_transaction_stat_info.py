...
def generate_xls(user_id, create_at_start, create_at_end, query_date):
    '''日期 支付笔数 支付成功金额 退款笔数 退款金额 交易总净额 优畅分润 银行分润'''
    wb = Workbook()
    ...
    inlet_info_data = db_transaction_stats(
        create_at_start_search, create_at_end_search, query_date)
...
def db_transaction_stats(create_at_start, create_at_end, query_date):
    query = """select
                {switch}
               sum(day_tx_count),
               ...
               round(sum(p_tranamt),2),
               round(sum(bk_tranamt),2)
    ...
    ret = [(d[0], int(d[1]), d[2] / 100, int(d[3]), abs(d[4]) / 100, d[5] / 100, d[6] / 100, d[7] / 100) for d in
           ret] if ret else []
...
def gen_inlet_info(ws, datas):
    fields = [u'日期', u'支付笔数',
              u'支付成功金额(元)', u'退款笔数', u'退款金额(元)', u'交易总净额(元)', u'优畅分润(元)', u'银行分润(元)']
...
def deal_search_count_details(search_count_details):
    ...
    for pay_start_time, day_tx_count, day_tx_amount, day_refund_count, day_refund_amount, day_tx_net_amout, day_profit_amount, day_bk_profit_amount in search_count_details:
        if not date_range_default[pay_start_time]:
            date_range_default[pay_start_time].extend(
                [pay_start_time, day_tx_count, day_tx_amount, day_refund_count, day_refund_amount, day_tx_net_amout,
                 day_profit_amount, day_bk_profit_amount])
        else:
            ...
            date_range_default[pay_start_time][7] += day_bk_profit_amount
