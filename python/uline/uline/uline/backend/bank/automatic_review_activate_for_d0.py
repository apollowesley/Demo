...
from uline.settings import (
    ...
    WX_APP_PRIVATE_KEY, WX_APP_ROOT_CA, ALI_PID, ALI_APPID, ALI_SERVER_ADDRESS,
    WX_0_APPID, WX_0_WX_MCH_ID, WX_0_WXPAY_KEY,
    WX_0_PRIVATE_KEY, WX_0_PUB_KEY, WX_0_ROOT_CA,
)
...
logger = logging.getLogger('my_auto_review_d0')
...
def automatic_review(mch_ids):
    failed_ids = []
    failed_inlet_3party = []
    success_inlet_mch_ids = []
    for mch_id in mch_ids:
        logger.info('auto_review mch_id:%s' % mch_id)
        ...
        with db.get_db() as cur:
            try:
                # 只审核d0商户
                # d0_mch = is_d0(mch_id, cur)
                # if not d0_mch:
                #     logger.info('1 not d0_mch:%s' % mch_id)
                #     continue
                ...
                if not changed_withdraw or not d0_mch:
                    logger.info('1 not d0_mch:%s' % mch_id)
                    continue
                ...
                # 只审核d0商户
                if not is_d0_mch:
                    logger.info('2 not d0_mch:%s' % mch_id)
                    continue
                ...
                wx_app_flag, msg_app_wx = mch_inlet_to_wx_app(
                    mch_id, cur, wx_app_flag, mch_wx_app_payment, wx_ali_id, mch_inlet_info, wx_app_channel_id, update_at)
                ...
                if not (wx_flag and wx_app_flag and ali_flag):
                    ...
                    result = yield update_changes(mch_id, cur, update_at)
                    db_payments = get_all_db_payments(cur, mch_id)
                    has_wx_d0 = False
                    has_dine = False
                    for each_payment in db_payments:
                ...
                if mch_status == 2:
                    success_inlet_mch_ids.append(mch_id)
                #     # 如果审核通过，则激活支付方式
    ...
    raise gen.Return(success_inlet_mch_ids)
...
def callback_and_nofity_mch(mch_ids):
    with db.get_db() as cur:
        for mch_id in mch_ids:
            ...
            notify_mch(cur, mch_id, datetime.now())
...
# 获取d0未审核商户
def get_dt_un_auth_d0_mch(dt_id):
    with db.get_db() as cur:
        sql = """select mch_inlet_info.mch_id from mch_inlet_info
                INNER JOIN mch_payment on mch_payment.mch_id = mch_inlet_info.mch_id
                where mch_inlet_info.dt_id=%s and mch_inlet_info.auth_status='1';"""
...
# 获取d0审核通过，未激活的商户
def get_dt_un_activate_d0_mch(dt_id):
    with db.get_db() as cur:
        sql = """select mch_inlet_info.mch_id from mch_inlet_info
                INNER JOIN mch_payment on mch_payment.mch_id = mch_inlet_info.mch_id
                where mch_inlet_info.dt_id=%s and mch_inlet_info.auth_status='2' and mch_payment.payment_type > 100
                and mch_inlet_info.activated_status='1';"""
        cur.execute(sql, (dt_id, ))
        mch_ids = cur.fetchall()
        return [mch_id[0] for mch_id in mch_ids]
...
def update_changes(mch_id, cur, update_at):
    ...
    logger.info('query_change_record_json:%s' % query_change_record_json)
    # 验证数据的合法性
    if query_change_record_json:
        ...
        if isinstance(query_change_record, dict):
            ...
            if new_mobile:
                old_mobile = get_mch_user_mobile(mch_id, cur)
                result = yield auth_password(mch_id, cur, str(old_mobile))
                # 如果reesult为True,说明该渠道商还未登录，mobile已经被修改了
                if result:
                    # 将密码修改为新的联系电话
                    new_password = common.bcrypt_pwd_new(str(new_mobile))
                    _query = """update mch_user set password=%s where mch_id=%s;"""
                    cur.execute(_query, (new_password, mch_id))

            logger.info('change_inlet_info:%s' % change_inlet_info)
        ...
        # return inlet_activated_status
        raise gen.Return(inlet_activated_status)


def get_mch_user_mobile(mch_id, cur):
    sql = """select mobile from mch_inlet_info where mch_id=%s;"""
    cur.execute(sql, (mch_id,))
    ret = cur.fetchone()
    return ret[0]
...
@gen.coroutine
def auth_password(mch_id, cur, password):
    valid = False
    selSql = """select password from mch_user where mch_id=%s;"""
    cur.execute(selSql, (mch_id,))
    db_ret = cur.fetchone()
    if db_ret:
...
def main():
    ...
    all_auth_mch_ids = []
    for dt_id in dt_ids:
        mch_ids = set(get_dt_un_auth_d0_mch(dt_id))
        print len(mch_ids)
        if mch_ids:
            mch_ids = [str(mch_id) for mch_id in mch_ids]
            success_inlet_mch_ids = yield automatic_review(mch_ids)
            # 如果审核成功
            if success_inlet_mch_ids:
                all_auth_mch_ids.extend(success_inlet_mch_ids)
    ...
    if all_auth_mch_ids:
        ...
        # 判断时间是否超过10分钟
        diff = (timedelta(minutes=1) - (end_time - begin_time)).seconds
        if diff > 0:
            time.sleep(diff)

        with db.get_db() as cur:
            for mch_id in all_auth_mch_ids:
                has_active_payments = automatic_activate(cur, mch_id, datetime.now())
