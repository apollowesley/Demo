...
from uline.handlers.app.bank.subsidize.form import MchSubsidizeAddForm
...
class MchSubsidizeAddHandler(BkAdminHandler):
    @authenticated
    @check_permission
    def prepare(self):
        self.form = MchSubsidizeAddForm(self)
        ...
        with uline_session_scope() as session:
            ...
            if role_inlet_info and role_type:
                ...
                inlet_info = role_inlet_info.to_dict()
                if subsidize_info:
        ...
        if not inlet_info or not is_bank_direct:
            ...
            return
        if subsidize_info:
            f_rsp = common.f_rsp(code=406, msg=u'该商户已是活动商户')
            self.finish(f_rsp)
            return
    ...
    def post(self):
        ...
        with uline_session_scope() as session:
            session.add(Discount(**discount_info))
