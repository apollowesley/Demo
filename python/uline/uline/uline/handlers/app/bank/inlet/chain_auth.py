...
from uline.public.constants import AUTH_STATUS, new_payment_relations
...
class AuthChainInletStatusHandler(BkAdminHandler):
            ...
            try:
                ...
                for payment_type in changed_payments:
                    ...
                    try:
                        ...
                        payment_type = int(new_payment_relations.get((payment.get('pay_type'))))
                ...
                if dt_inlet_info and dt_inlet_info[9]:
                    ...
                    # wx_channel_id, wx_app_channel_id = self.get_chain_parent_info(cur, dt_inlet_info[9])
                ...
                # parent_type = parent_info[3]
                # if parent_type == 2 and not alipay_pid:
                if not alipay_pid:
                    alipay_pid = ALI_PID
    ...
    def update_changes(self, cur):
        ...
        if query_change_record:
            ...
            need_update_keys = ['dt_name', 'province', 'city', 'address', 'contact', 'mobile', 'service_phone', 'email',
                                ...
                                'wx_ind_code', 'ali_ind_code', 'dt_short_name',
                                'license_type', 'head_name', 'head_mobile', 'head_type'
                                ]
    ...
    def get_dt_inlet_info(self, cur):
        query = """select dt_name, contact, mobile, email, old_ind_code, wx_ind_code, service_phone, wx_channel_id,
                          wx_app_channel_id, parent_id, dt_short_name, ali_ind_code, address, province, city, district,
    ...
    def add_fail_auth_inlet_info(self, cur, show_comment):
        auth_user = self.get_bk_email(cur)
        query = """
            insert into auth_dt_info (
            dt_id,
            comment,
    ...
    def get_chain_parent_info(self, cur, parent_id):
        query = """SELECT wx_channel_id, wx_app_channel_id, alipay_pid, dt_type FROM dt_inlet_info where dt_id=%s"""
    ...
    def update_cs_wx_sub_id(self, cur):
        ...
        params.append(self.dt_id)
        sql = """UPDATE mch_user SET {}
                 WHERE EXISTS
                     (
                         select 1 from mch_inlet_info
                         where mch_inlet_info.mch_id=mch_user.mch_id and mch_inlet_info.cs_id=%s
                     )"""
