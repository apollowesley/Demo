...
from .form import ActivatedDistributorInfo
from uline.public import log
from uline.public.constants import ACTIVATED_STATUS, new_payment_relations
...
class ActivatedDistributorInletStatusHandler(BkAdminHandler):

    @authenticated
    @check_permission
    def prepare(self):
        form = ActivatedDistributorInfo(self)
        self.f_rsp = common.f_rsp(code=406, msg='fail')

        if not form.validate():
            self.finish(self.f_rsp)
        self.dt_id = form.dt_id.data
        self.uline_payment_code = form.pay_type.data
...
客服电话：4008047555""".format(str(self.dt_info[2]) + ".dt", self.dt_info[1], self.dt_info[2], DIST_LOGIN_URL, addition_info)
    ...
    def activated_dt_payment(self, cursor):
        query = """update dt_payment set
            ...
            where dt_id=%(dt_id)s and uline_payment_code=%(uline_payment_code)s"""
        cursor.execute(query, {
            "activated_status": self.activated_status,
            "dt_id": self.dt_id,
            "update_at": self.update_at,
            "uline_payment_code": self.uline_payment_code
        })
