...
class MerchantInletIndexHandler(BkAdminHandler):
    ...
    def get(self):
        ...
        self.render('bank/inlet/merchant.html', navigate_html=navigate_html, data=self.data, form=self.form,
    ...
    def db_execute(self, form, offset):
        ...
        query = """SELECT mch.mch_id,mch.mch_name,mch.mch_shortname,bk_user.bk_type,mch.activated_status,mch.auth_status,dt.dt_name,z.auth_user, COUNT(*) OVER () AS total
                from mch_inlet_info as mch
                  ...
                  inner join (select auth_user ,mch_id from auth_mch_info as a1 where not exists(select 1 from auth_mch_info as a2
                    ...
                    INNER JOIN bk_user on bk_user.bk_id=mch.bk_id
                ...
                AND mch.bk_id IN (SELECT bk_id FROM bk_user WHERE (bk_type = %(expand_type)s OR %(expand_type)s IS NULL)
        ...
        if ret:
            ...
            for i in ret:
                ...
                if self.open_review:
                    ...
                    # not_same_user = 0 if last_auth_user == current_user else 1
    ...
    def get_last_auth_user(self, mch_id):
        ...
        auth_mch_info = uline_session.query(AuthMchInfo).filter(
            AuthMchInfo.mch_id == mch_id).order_by(desc(AuthMchInfo.create_at)).first()
        if auth_mch_info:
