...
from uline.settings import (
    WEIXIN_REFUND_URL, ALIPAY_REFUND_URL, JD_REFUND_URL
)
from uline.public.permit import check_permission


class MerchantsRefundDetailHandler(MchAdminHandler):
    ...
    def get(self):
        ...
        self.render('merchant/transaction/singleMerchantRefund.html', data=data)
...
class MerchantsInitiateRefundHandler(MchAdminHandler):
    ...
    def prepare(self):
        ...
        self.mch_id = self.get_current_user()
    ...
    def post(self):
        ...
        if not validation_password:
            ...
            if not mch_pay_key or not self.refund_fee:
                ...
                # 添加京东退款
                if data.get('channel') == 'weixin':
                    url = WEIXIN_REFUND_URL
                elif data.get('channel') == 'alipay':
                    url = ALIPAY_REFUND_URL
                else:
                    url = JD_REFUND_URL
