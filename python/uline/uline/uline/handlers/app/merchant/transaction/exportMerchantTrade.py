...
from uline.backend.merchant.generate_mch_trade_info import generate_xls
import tcelery
...
class ExportMerchantTradeHandler(MchAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        file_type = self.form.file_type.data or 'csv'
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, complete_at_start, complete_at_end,
                                            out_trade_no, mch_trade_no, wx_transaction_id,
                                            trade_state, trade_type, channel, total_num, file_type])
