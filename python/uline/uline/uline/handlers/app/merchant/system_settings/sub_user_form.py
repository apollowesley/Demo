...
import uline.handlers.app.merchant.utils.form as mch_form_utils
...
def validate_login_name(form, field):
    ...
    if str(mch_id_from_user) != str(form.mch_id):
...
class SubUserIndexForm(form_utils.BaseForm):
    ...
    mch_sub_name = fields.StringField(validators=[validators.Optional()])
...
class SubUserEditForm(SubUserForm):
    ...
    mch_sub_id = fields.IntegerField(validators=[
        validators.DataRequired(
            message=u'子账户id不能为空'), mch_form_utils.validate_mch_sub_id
    ])
