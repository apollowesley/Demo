...
from uline.backend.dist.generate_mch_refund_info import generate_xls
import tcelery
...
class ExportMerchantRefundHandler(DtAdminHandler):
    ...
    def prepare(self):
        ...
        if not self.form.validate():
            self.redirect('/dist/transaction/refund')
    ...
    def db_inlet_info(self):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, mch_name, mch_id, create_at_start, create_at_end,
