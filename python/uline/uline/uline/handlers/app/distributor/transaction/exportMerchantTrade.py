...
from uline.backend.dist.generate_mch_trade_info import generate_xls
import tcelery
...
class ExportMerchantTradeHandler(DtAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, mch_name, mch_id, complete_at_start, complete_at_end,
                                            out_trade_no, mch_trade_no, wx_transaction_id,
                                            trade_state, trade_type, channel, total_num, dt_sub_id, cs_mch_shortname,
                                            cs_mch_id])
            ret = response.result
        else:
