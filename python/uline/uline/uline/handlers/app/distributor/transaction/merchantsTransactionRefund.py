...
class MerchantsTransactionRefundHandler(DtAdminHandler):
    ...
    def prepare(self):
        ...
        if not self.form.validate():
            self.redirect('/distributor/transaction/refund')
            return
    ...
    def get(self):
        ...
        self.render('distributor/transaction/merchantRefund.html', navigate_html=navigate_html, data=data,
        ...
        for tb_data in ret:
            for ul_data in ul_ret:
                if int(tb_data[0]) == ul_data[0]:
                    tb_data.extend(
                        [ul_data[1], ul_data[2], ul_data[3], ul_data[4], ul_data[5]])
                    data.append(tb_data)
