...
from uline.backend.dist.generate_transaction_stat_info import generate_xls
import tcelery
...
class ExportTransactionStatisticHandler(DtAdminHandler):
    ...
    def get(self):
        ...
        yield self.async_get_xls()

    @gen.coroutine
    def async_get_xls(self):
        xls_path = yield self.db_inlet_info()
        if xls_path:
    ...
    def db_inlet_info(self):
        ...
        # if create_at_start:
        #     create_at_start += timedelta(days=1)
        # if create_at_end:
        #     create_at_end += timedelta(days=2)

        response = yield gen.Task(generate_xls.apply_async, args=[dt_id, create_at_start, create_at_end, query_date])
        ret = response.result
