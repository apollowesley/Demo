...
class TransactionStatisticsHandler(DtAdminHandler):
    ...
    def get(self):
        ...
        self.render('distributor/statistics/transactionStatistics.html', data=data, order_by_form=self.order_by, query_date=int(
            self.query_date), create_at_start=self.create_at_start, create_at_end=self.create_at_end)
    ...
    def get_pay_count(self, cursor, create_at_start=None, create_at_end=None):
        ...
        cursor.execute(query, {'create_at_start': create_at_start,
                               'create_at_end': create_at_end, 'dt_id': self.current_user})
        ret = cursor.fetchone()
