...
from uline.backend.dist.generate_cs_inlet_info import generate_xls
import tcelery
...
class ExportCsHandler(DtAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        cs_name = self.form.cs_name.data or None
        total_num = self.form.total_num.data or 0
        cs_id = self.form.cs_id.data or None
        mch_id = self.form.mch_id.data or None

        # 因为end是0时0分,将它续一天┑(￣Д ￣)┍
        create_at_start = "2014-01-01" if not create_at_start else create_at_start
        create_at_end = create_at_end + \
            timedelta(days=1) if create_at_end else datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, cs_name, short_name, cs_id, mch_id, create_at_start,
            ...
            if d_num < DOWNLOAD_FILE_NUM_LIMIT:
                yield gen.Task(generate_xls.apply_async, args=[self.current_user, cs_name, short_name, cs_id, mch_id, create_at_start,
                                                               create_at_end, activated_status,
                                                               auth_status, total_num])
                ret = {'order': True}
            else:
