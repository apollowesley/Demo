...
from uline.backend.dist.generate_dist_balance_info import generate_xls
import tcelery
...
class ExportDistributorBalanceHandler(DtAdminHandler):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            ...
            if d_num < DOWNLOAD_FILE_NUM_LIMIT:
                yield gen.Task(generate_xls.apply_async,
                               args=[self.current_user, create_at_start, create_at_end, pay_status, total_num, channel])
                ret = {'order': True}
            else:
                ret = {'over_order': True}
