...
class BkIndexHanddler(IntBkAdminHandler):

    @authenticated
    @check_permission
    def get(self):
        self.render('inter_bank/account/index.html')


class BkLoginHanddler(IntBkAdminHandler):

    def prepare(self):
        self.form = LoginForm(self)
        if self.request.method == 'POST':
            if not self.form.validate():
                self.redirect('/inter_bank/login')
            else:
    ...
    def get(self):
        if self.get_current_user():
            ...
            return
        self.render('inter_bank/account/login.html', passwdMsg='', form=self.form)
    ...
    def post(self):
        ...
        if db_ret:
            if not use_secure_cookie:
                self.session['inter_bk_id'] = db_ret[0]
                self.session['inter_bk_name'] = db_ret[2]
                self.session.save()
            else:
                # expires = datetime.datetime.now() + datetime.timedelta(minutes=60)
                # 一天有效期
                self.set_secure_cookie('inter_bk_id', str(db_ret[0]), expires_days=1)
                self.set_secure_cookie('inter_bk_name', db_ret[2], expires_days=1)
            self.redirect('/inter_bank/')
        else:
            # 暂时注释银行子帐号登录
            # bk_user 中找不到记录
            ...
            self.render('inter_bank/account/login.html', passwdMsg=u'账号或密码错误', form=self.form)

    @coroutine
    def auth_user(self, email, password):
        valid = False
        selSql = "select bk_id,password,bk_name from bk_user where email=%s and bk_type=2"
...
class BkLogoutHanddler(IntBkAdminHandler):

    @authenticated
    def get(self):
        self.session['inter_bk_id'] = None
        self.session['bk_sub_id'] = None
        self.session.save()

        self.clear_cookie('inter_bk_id')
        self.clear_cookie('bk_sub_id')

        self.redirect('/inter_bank/login')


class BkResetPasswordHandler(IntBkAdminHandler):
    ...
    def get(self):
        self.render('inter_bank/cgPassword.html')
    ...
    def post(self):
        ...
        if bk_sub_id:
            if self.auth_subuser_email(email, bk_sub_id):
                self.redirect('/inter_bank/resetpasswd/auth')
                return
        else:
    ...
    def auth_email(self, email, bk_id):
        return self.db.selectSQL("select 1 from bk_user where email=%s and bk_id=%s and bk_type=2", (email, bk_id))
...
class BkResetPasswordAuthHandler(IntBkAdminHandler):

    @authenticated
    def get(self):
        self.render('inter_bank/cgPassword2.html')
    ...
    def post(self):
        ...
        if self.check_auth_code(auth_code):
            self.redirect('/inter_bank/resetpasswd/modify')
            return
        self.render('inter_bank/cgPassword2.html')
...
class BkResetPasswordAuthCodeHandler(IntBkAdminHandler):
    ...
    def get_ub_user_email(self, bk_id):
        return self.db.selectSQL("select email from bk_user where bk_id=%s and bk_type=2", (bk_id,))

    def get_ub_subuser_email(self, bk_sub_id):
        return self.db.selectSQL("select email from bk_subuser where bk_sub_id=%s", (bk_sub_id, ))


class BkResetPasswordModifyHandler(IntBkAdminHandler):

    @authenticated
    def get(self):
        self.render('inter_bank/cgPassword3.html')
    ...
    def post(self):
        ...
        if not form.validate() and not (new_pwd == check_pwd):
            self.render('inter_bank/cgPassword3.html', form=form)
            return
