...
from uline.public.constants import ACTIVATED_STATUS, PAYMENT, new_payment_relations
...
class ActivatedMerchantInletStatusHandler(IntBkAdminHandler):
...
客服电话：4008047555""".format(str(self.mch_info[2]) + ".mch", self.mch_info[1], self.mch_info[2], self.mch_info[3], MCH_LOGIN_URL,
    ...
    def get_dt_activated_status(self, cursor):
        query = """SELECT activated_status FROM dt_payment
                    WHERE dt_id=(SELECT dt_id FROM mch_inlet_info WHERE mch_id=%s) and payment_type=%s;"""
    ...
    @gen.coroutine
    def add_activated_mch_info(self, cursor, activated_status=None):
