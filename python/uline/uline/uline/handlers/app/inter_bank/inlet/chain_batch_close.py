...
from .form import CloseBatchChainPayment
from uline.backend import tasks
from uline.public.permit import check_permission


class CloseBatchChainPaymentHandler(IntBkAdminHandler):

    @authenticated
    @check_permission
    def prepare(self):
        form = CloseBatchChainPayment(self)
    ...
    def get_mch_ids_by_dt_id(self, cursor):
        query = """select DISTINCT(mch_id) FROM mch_inlet_info WHERE cs_id=%(dt_id)s;"""
        cursor.execute(query, {
            'dt_id': self.dt_id,
        })
        ret = cursor.fetchall()
