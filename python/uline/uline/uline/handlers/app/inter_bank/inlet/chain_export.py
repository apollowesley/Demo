...
from uline.backend.inter_bank.generate_chain_inlet_info import generate_xls
import tcelery
...
class ExportChainHandler(IntBkAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        parent_id = self.form.parent_id.data or None
        dt_id = self.form.dt_id.data or None
        parent_name = self.form.parent_name.data or None
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async, args=[self.current_user, dt_name, dt_id, create_at_start,
                                                                      create_at_end, activated_status,
                                                                      auth_status, total_num, parent_id, parent_name])
            ret = response.result
        else:
