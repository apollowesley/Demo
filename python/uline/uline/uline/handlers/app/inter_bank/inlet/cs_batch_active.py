...
class ActivatedBatchCsInletStatusHandler(IntBkAdminHandler):
    ...
    def get(self):
        ...
        with self.db.get_db() as cur:
            try:
                ...
                chain_unactivated_payment_type = yield self.get_chain_unactivated_payment_type(cur)
                if chain_unactivated_payment_type:
                    payment_name = '、'.join(PAYMENT[str(payment_type[0])]
                                            for payment_type in chain_unactivated_payment_type if payment_type in payment_types)
                    if payment_name:
                ...
                yield self.add_activated_cs_info(cur, payment_types)
...
客服电话：4008047555""".format(self.mch_info[0], self.mch_info[1], self.mch_info[2], self.mch_info[3], MCH_LOGIN_URL,
    ...
    @gen.coroutine
    def get_chain_unactivated_payment_type(self, cursor):
        query = """SELECT payment_type FROM dt_payment
                    WHERE dt_id=(SELECT cs_id FROM mch_inlet_info
    ...
    @gen.coroutine
    def add_activated_cs_info(self, cursor, payment_types):
