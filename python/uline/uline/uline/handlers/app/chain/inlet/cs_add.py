...
# @author: liufei
# @createTime: 2017-06-07 19:44:26
# @description: 添加连锁门店
...
class AddCsInletHandler(ChainAdminHandler):
    ...
    def prepare(self):
        ...
        self.dt_id = chain_utils.get_dt_id_for_chain_id(self.current_user)[0]
    ...
    def get_chain_api_key(self):
        ...
        api_key = self.db.selectSQL(query, (self.dt_id, ))[0]
        if not api_key:
            api_key = common.gen_randown_mch_pkey(32)
            query = """update dt_user set api_key=%s where dt_id=%s;"""
            self.db.executeSQL(query, (api_key, self.dt_id))
        return api_key
