...
class NewsOperateWeb(ChainAdminHandler):
    # /sendnotify
    @authenticated
    @check_permission
    def prepare(self):
    ...
    def get(self):
        # 开通为2,关闭为3,未开通为初始值1
        ret = uline_session.query(DtInletInfo.chain_wechat_news).filter(
            DtInletInfo.dt_id == self.query_id).one()
        if ret[0] != 2:
            self.render('chain/system_settings/weChatnews.html',
            ...
            notify_payment_type = uline_session.query(DtPayment.payment_type, DtPayment.news_push_open_status).order_by(
                DtPayment.payment_type).filter(DtPayment.dt_id == self.query_id).all()
            payment_type = []
...
class SwitchWechatNews(ChainAdminHandler):
    # /open
    @authenticated
    def prepare(self):
    ...
    def get(self):
        ...
        try:
            uline_session.query(DtInletInfo).filter(DtInletInfo.dt_id == self.query_id).update(
...
class ChoosePaymentType(ChainAdminHandler):
    ...
    def get(self):
        ...
        try:
            query = uline_session.query(DtPayment.news_push_open_status).filter(
                DtPayment.dt_id == self.query_id, DtPayment.payment_type == self.payment_type).one()
            if query[0] == int(self.open_status):
                rsp = common.f_rsp(code=406, msg='same_status')
            else:
                uline_session.query(DtPayment).filter(DtPayment.dt_id == self.query_id,
