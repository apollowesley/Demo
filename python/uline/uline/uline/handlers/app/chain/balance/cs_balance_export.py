...
from uline.backend.chain.generate_mch_balance_info import generate_xls
import tcelery
...
class CsBalanceExportHandler(ChainAdminHandler):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, mch_name, mch_id, create_at_start,
                                            create_at_end, pay_status, total_num, channel, chain_sub_id])
            ret = response.result
        else:
            ...
            if d_num < DOWNLOAD_FILE_NUM_LIMIT:
                yield gen.Task(generate_xls.apply_async, args=[self.current_user, mch_name, mch_id, create_at_start,
                                                               create_at_end, pay_status, total_num, channel,
                                                               chain_sub_id])
                ret = {'order': True}
            else:
                ret = {'over_order': True}
