...
from uline.backend.chain.generate_mch_trade_info import generate_xls
import tcelery
...
class CsExportTradeHandler(ChainAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            ...
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, self.dt_id, mch_name, mch_id, complete_at_start,
                                            complete_at_end,
                                            out_trade_no, mch_trade_no, wx_transaction_id,
                                            trade_state, trade_type, channel, total_num, chain_sub_id])
            ret = response.result
        else:
            d_num = yield self.get_download_num()
            if d_num < DOWNLOAD_FILE_NUM_LIMIT:
                yield gen.Task(generate_xls.apply_async,
                               args=[self.current_user, self.dt_id, mch_name, mch_id, complete_at_start, complete_at_end,
                                     out_trade_no, mch_trade_no, wx_transaction_id,
                                     trade_state, trade_type, channel, total_num, chain_sub_id])
                ret = {'order': True}
            else:
