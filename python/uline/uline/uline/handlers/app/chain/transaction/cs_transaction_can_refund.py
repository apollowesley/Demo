...
class CsTransactionCanRefundHandler(ChainAdminHandler):
    ...
    def has_refund_password_or_not(self):
        has_refund_query = """select refund_password from dt_user where dt_id=%(chain_id)s"""
        ret = self.db.selectSQL(has_refund_query, {'chain_id': self.current_user})
