...
from uline.backend.chain.generate_mch_refund_info import generate_xls
import tcelery
...
class CsExportRefundHandler(ChainAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, self.dt_id, mch_name, mch_id, create_at_start, create_at_end,
                                            mch_trade_no, out_trade_no, transaction_id, refund_status,
                                            mch_refund_no, out_refund_no, refund_id,
                                            channel, total_num, chain_sub_id])
            ret = response.result
        else:
