...
def validate_mch_id(form, field):
    ret = db.selectSQL("SELECT 1 FROM mch_inlet_info WHERE mch_id=%s", (field.data,))
    if not ret:
...
class OpenCloseWechatNews(BaseForm):
    open_status = fields.IntegerField(validators=[validators.required(), validators.any_of([1, 2, 3])])


class ChoosePayment(BaseForm):
    open_status = fields.IntegerField(validators=[validators.required(), validators.any_of([2, 3])])
    payment_type = fields.IntegerField(
        validators=[validators.required(), validators.any_of([1, 2, 3, 4, 5, 6, 7, 8, 9])])
