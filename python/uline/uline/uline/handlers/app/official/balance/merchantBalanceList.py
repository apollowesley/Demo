...
class MerchantBalanceIndexListHandler(OfclAdminHandler):
    ...
    def prepare(self):
        ...
        if not form.validate():
            self.redirect('/official/balance/mch/list')
            return
    ...
    def get(self):
        ...
        cs = {"cs_id": self.cs_id, "cs_dt_name": self.cs_dt[0], "cs_dt_shortname": self.cs_dt[1]} if self.cs_id else {
            "cs_id": self.cs_id}
        data.update(cs)
        self.render('official/balance/merchantBalanceList.html', data=data)
    ...
    def do_execute(self):
        ...
        fields = ['create_at', 'mch_id', 'need_pay_time', 'mch_name', 'mch_shortname', 'channel', 'dt_name', 'rcvAcctName', 'balance_account', 'rcvBankName',
