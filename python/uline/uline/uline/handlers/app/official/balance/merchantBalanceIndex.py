...
class MerchantBalanceIndexIndexHandler(OfclAdminHandler):
    ...
    def get(self):
        ...
        self.render('official/balance/merchantBalance.html', navigate_html=navigate_html, data=self.data,
    ...
    def db_execute(self, form, offset):
        ...
        if not create_at_start:
            create_at_start = datetime.min

        # 因为end是0时0分,将它续一天┑(￣Д ￣)┍
        if create_at_end:
            create_at_end += timedelta(days=1)
        else:
            create_at_end = datetime.max
