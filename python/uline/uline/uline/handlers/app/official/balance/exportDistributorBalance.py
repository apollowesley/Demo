...
from uline.backend.official.generate_dist_balance_info import generate_xls
import tcelery
...
class ExportDistributorBalanceHandler(OfclAdminHandler):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, dt_name, dt_id, create_at_start,
                                            create_at_end, pay_status, total_num, channel, balance_account, expand_type, bk_id, bk_name])
            ret = response.result
        else:
