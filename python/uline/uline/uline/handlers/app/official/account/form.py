...
class ModifyPasswordForm(BaseForm):
    newPwd = fields.StringField(validators=[validators.length(
        min=5, max=20), validators.required(message=u'密码不能为空')])
    checkPwd = fields.StringField(validators=[validators.length(
        min=5, max=20), validators.required(message=u'确认密码不能为空')])
