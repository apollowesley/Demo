...
from uline.backend.official.generate_mch_refund_info import generate_xls
import tcelery
...
class ExportMerchantRefundHandler(OfclAdminHandler):
    ...
    def prepare(self):
        ...
        if not self.form.validate():
            self.redirect('/official/transaction/refund')
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, dt_name, mch_name, dt_id, mch_id, create_at_start,
                                            create_at_end, out_trade_no, mch_trade_no, transaction_id, refund_status,
