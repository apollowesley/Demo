...
from uline.backend.bank.generate_dist_statistics import generate_xls
import tcelery
...
class ExportDistStatistics(OfclAdminHandler):
    ...
    def prepare(self):
        ...
        if not self.form.validate():
            self.redirect('/official/stats/dist')
            return
        ...
        if int(query_date) == 2:
            ...
            self.create_at_end = datetime(int(_year),
                                          month_, 1, 23, 59, 59).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def async_get_xls(self):
        ret = yield self.db_statistics_info()
    ...
    def db_statistics_info(self):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async,
                                      args=[self.current_user, dt_name, dt_id, create_at_start, create_at_end,
                                            order_by, total_num, expand_type, bk_id, bk_name])

            # response = generate_xls(self.current_user, dt_name, dt_id, create_at_start, total_num)
            ret = response.result
        else:
