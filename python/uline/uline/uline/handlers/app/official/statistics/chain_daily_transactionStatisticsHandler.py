...
class ChainDailyTransactionHandler(OfclAdminHandler):
    ...
    def get(self):
        ...
        self.render('official/statistics/chaintransactionStatistics.html', data=data, order_by_form=self.order_by,
    ...
    def do_execute(self):
        ...
        with self.db.get_db() as cur:
            try:
                last_day_count = yield self.get_pay_count(cur, self.chain_id, self.last_create_at_start,
                                                          self.last_create_at_end)
    ...
    def get_pay_count(self, cursor, chain_id=None, create_at_start=None, create_at_end=None):
        ...
        query = """select
                ...
                left join mch_inlet_info on mch_inlet_info.cs_id = ci.cs_id
    ...
    def get_pay_count_detail(self, cursor, chain_id, create_at_start, create_at_end, offset, page=False):
        ...
        query = """select
                ...
                COUNT(*) over () as total
                from cs_daily_balance_info AS ci
                left join mch_inlet_info on ci.cs_id = mch_inlet_info.cs_id
                where (ci.cs_id = %(chain_id)s or %(chain_id)s is null)
    ...
    #     sort_num = ['need_pay_time', 'day_tx_count', 'day_tx_amount', 'day_refund_count',
    #                 'day_refund_amount', 'day_tx_net_amout', 'day_profit_amount']
    #     deal_details = list()
    #     for d in details:
    #         deal_details.append(tuple(d))
    #     raise gen.Return(sorted(deal_details, key=lambda tuple_e: tuple_e[sort_num.index(self.order_by)], reverse=True))
