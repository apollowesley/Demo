...
class MchDailyTransactionHandler(OfclAdminHandler):
    ...
    def get(self):
        ...
        self.render('official/statistics/mchtransactionStatistics.html', data=data, order_by_form=self.order_by, query_date=int(self.query_date),
        ...
        with self.db.get_db() as cur:
            try:
                ...
                last_day_count = yield self.get_pay_count(cur, self.mch_id, self.last_create_at_start,
                                                          self.last_create_at_end)
                # 累计汇总 模块上
                total_count = yield self.get_pay_count(cursor=cur, mch_id=self.mch_id, create_at_start=None,
                ...
                total_count_search = yield self.get_pay_count(cur, self.mch_id, total_query_start,
                                                              self.create_at_end_search)
                # 曲线详情
                search_count_details = yield self.get_pay_count_detail(cur, self.mch_id, self.create_at_start_search,
                                                                       self.create_at_end_search,
                                                                       (self.pageindex - 1) *
                                                                       10, self.query_date,
                                                                       page=True)
                # 曲线图
                chart_show_details = yield self.get_pay_count_detail(cur, self.mch_id, self.create_at_start_search,
