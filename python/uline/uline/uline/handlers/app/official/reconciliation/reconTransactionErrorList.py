...
from .form import ReconErrorList
from uline.public.constants import RECON_HANDLE_STATUS, RECON_EXCEPT_TYPE
from uline.public.permit import check_permission


class ReconTransactionErrorListHandler(OfclAdminHandler):

    @tornado.web.authenticated
    @check_permission
    def prepare(self):
        form = ReconErrorList(self)
        if not form.validate():
            self.redirect('/official/balance/transaction/list')
            return
    ...
    def get(self):
        ...
        self.render('official/reconciliation/transactionList.html',
                    recon_error_html=recon_error_html, data=data)

    def do_execute(self):
        ret, detail = self.get_recon_transaction_error()
        tb_ret = self.get_order_transaction()
    ...
    def deal_detail(self, detail):
        format_detail = {'sub_mch_id': '子商户号', 'cash_fee_type': '现金支付货币类型', 'wx_transaction_id': '微信支付订单号',
                         'appid': 'Uine公众账号ID', 'time_end': '支付完成时间', 'bank_type': '付款银行',
                         'attach': '附加数据', 'mch_id': '优畅商户号', 'out_trade_no': '商户订单号',
                         'total_fee': '订单金额', 'trade_state': '交易状态	'}
        detail_new = dict()
    ...
    def get_recon_transaction_error(self):
        query = """select
        ...
        ret = list(self.db.selectSQL(query, (self.out_trade_no,)))
        ...
        ret_d = dict(zip(fields, ret))
        ret_d['out_trade_no'] = self.out_trade_no
        return ret_d, detail

    def get_order_transaction(self):
        default_ret = [''] * 2
        query = """select ul_mch_id,wx_transaction_id from orders where out_trade_no=%s;"""
        ret = self.tdb.selectSQL(query, (self.out_trade_no,))
        fields = ['mch_id', 'wx_transaction_id']
    ...
    def deal_detail_total_fee(self, detail):
        total_fee = detail.get('total_fee')
        if total_fee:
