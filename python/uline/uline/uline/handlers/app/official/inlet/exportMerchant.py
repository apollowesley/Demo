...
from uline.backend.official.generate_mch_inlet_info import generate_xls
import tcelery
...
class ExportMerchantHandler(OfclAdminHandler):
    ...
    def db_inlet_info(self):
        ...
        if total_num <= DOWNLOAD_INFO_NUM_LIMIT:
            response = yield gen.Task(generate_xls.apply_async, args=[self.current_user, dt_name, mch_name, dt_id, mch_id, create_at_start,
            ...
            if d_num < DOWNLOAD_FILE_NUM_LIMIT:
                yield gen.Task(generate_xls.apply_async, args=[self.current_user, dt_name, mch_name, dt_id, mch_id, create_at_start,
                                                               create_at_end, activated_status,
                                                               auth_status, total_num])
                ret = {'order': True}
            else:
