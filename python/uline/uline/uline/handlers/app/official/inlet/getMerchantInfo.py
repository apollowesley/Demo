...
from uline.public import common, constants, merchant_risk_util
from uline.utils.record import record_utils
from uline.settings import FEATURE_SWITCH
from uline.public.permit import check_permission


class GetMerchantInfoHandler(OfclAdminHandler):
    @authenticated
    @check_permission
    def prepare(self):
    ...
    def get(self):
        ...
        if result:
            ...
            record_utils.swap_value(result, s_rsp['data'], change_id=s_rsp['data']['mch_id'], role_type=2)
        ...
        subsidize_info = self.get_mch_subsidize_info(role_id, role_type)
        ...
        s_rsp['data']['risk_info'] = risk_info
        s_rsp['data'].update(risk_account_status)
    ...
    def get_new_payment(self):
        ret = self.db.selectSQL("""SELECT uline_payment_code,settle_rate, activated_status, uline_settle_id,
                                   payment_type, withdraw_fee, withdraw_rate
                                   FROM mch_payment WHERE mch_id=%s;""", (self.mch_id,),
                                fetchone=False)
        fields = ['uline_payment_code', 'settle_rate', 'pay_rate', 'activated_status', 'uline_settle_id', 'payment_type',
                  'pay_type',
                  'withdraw_fee', 'withdraw_rate']
        data = [dict(zip(fields, [i[0], i[1] / 10, i[1] / 10, i[2], i[3], i[4], i[4], i[5], i[6]]))
    ...
    def get_auth_info(self):
        ...
        data = [dict(zip(fields, [i[0], i[1], i[2], i[3]])) for i in ret] if ret else []
    ...
    def get_mch_subsidize_info(self, role_id, role_type):
        query = """select rate from discount where role_id=%s and role=%s and valid=1 order by create_at desc limit 1"""
        ret = self.db.selectSQL(query, (role_id, role_type), fetchone=True, use_dict=True)
        subsidize_info = {}
        if ret and ret['rate']:
    ...
    def get_mch_risk_info(self):
        query = """select merchant_risk_item.risk_type from merchant_risk_item
                ...
                """
        ret = self.db.selectSQL(query, (self.mch_id, 'cs' if self.is_cs else 'mch'), fetchone=False, use_dict=True)
    ...
    def get_mch_settle_status(self):
        ...
        result = self.db.selectSQL(query, (self.mch_id, 'cs' if self.is_cs else 'mch'), fetchone=True, use_dict=True)
        settle_status = {
            'weixin': 0,
            'alipay': 0,
        }
