    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if(res.code==200 && res.msg=="fsuccess"){
                ...
                $(tr + " td:nth-child(4)").html("复审中");
                $(tr + " td:nth-child(5)").html("").append("<div class='passBox'>初审通过</div>");
                ...
                // $(tr + " td:nth-child(3)").html("未激活");
                $(tr + " td:nth-child(4)").html("审核通过");
                $(tr + " td:nth-child(5)").html("").append("<div class='passBox'>复审通过</div>");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    openNewTable(value,1);
                },1000);
                ...
                // $(tr + " td:nth-child(3)").html("未激活");
                $(tr + " td:nth-child(4)").html("审核通过");
                $(tr + " td:nth-child(5)").html("").append("<div class='passBox'>审核通过</div>");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    judgeStatus(value);
                    openNewTable(value,1);
                ...
                // $(tr + " td:nth-child(3)").html("未激活");
                $(tr + " td:nth-child(4)").html("进件中");
                $(tr + " td:nth-child(5)").html("").append("<div class='passBox'>进件中</div>");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    judgeStatus(value);
                    openNewTable(value,1);
                ...
                $(tr + " td:nth-child(3)").html("未激活");
                $(tr + " td:nth-child(4)").html("审核不通过");
                $(tr + " td:nth-child(5)").html("").append("<div class='passBox' style='width: 300px'>进件到微信失败,系统已经自动驳回</div>");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    openNewTable(value,1);
                },1000);
...
function denyDtInletStatus(value) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if(res.code==200){
                $(tr + " td:nth-child(4)").html("审核不通过");
                $(tr + " td:nth-child(5)").children("a").remove();
                $('.modal').modal('hide');
                $(tr + " td:nth-child(5)").append("<div class='passBox'>审核驳回</div>");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    openNewTable(value,1);
                },1000);
...
function judgeStatus(value){
    $.ajax({
        ...
        data: {"dt_id": value},
        dataType: 'json',
        url: '/inter_bank/inlet/dt/detail',
        beforeSend: function (XMLHttpRequest) {
            // 请求前处理
        },
        success: function (res) {
            ...
            if(res.code==200){
                for (var index = 0; index < res.data.payment.length; index++) {
                    if(res.data.payment[index].activated_status==2){
                        $("#tr_"+value + " td:nth-child(3)").html("已激活");
                        return;
                    }
                }
                $("#tr_"+value + " td:nth-child(3)").html("未激活");
            }
        }
