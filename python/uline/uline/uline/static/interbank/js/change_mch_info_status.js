    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if(res.code==200 && res.msg=="fsuccess"){
                ...
                $(tr + " td:nth-child(6)").html("复审中").append("<div class='passBox'>初审通过</div>");
                ...
                // $(tr + " td:nth-child(5)").html("未激活");
                $(tr + " td:nth-child(6)").html("审核通过").append("<div class='passBox'>复审通过</div>");
                $(tr + " td:nth-child(7)").html("");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    openNewTable(value,1);
                ...
                // $(tr + " td:nth-child(5)").html("未激活");
                $(tr + " td:nth-child(6)").html("审核通过").append("<div class='passBox'>审核通过</div>");
                $(tr + " td:nth-child(7)").html("");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    judgeStatus(value);
                ...
                // $(tr + " td:nth-child(5)").html("未激活");
                $(tr + " td:nth-child(6)").html("进件中").append("<div class='passBox'>进件中</div>");
                $(tr + " td:nth-child(7)").html("");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    judgeStatus(value);
                ...
                $(tr + " td:nth-child(5)").html("未激活");
                $(tr + " td:nth-child(6)").html("审核不通过").append("<div class='passBox'>进件到微信或支付宝失败,系统已自动驳回</div>");
                $(tr + " td:nth-child(7)").html("");
                $(".passBox").delay(1000).fadeOut();
                setTimeout(function(){
                    openNewTable(value,1);
        ...
        error: function () {
            loadingHide();
            // 请求出错处理
            $(tr + " td:nth-child(6)").append("<div class='passBox'>操作失败</div>");
...
function denyMchInletStatus(value) {
    ...
    $.ajax({
        ...
        success: function (res) {
            // 请求成功处理
            if(res.code==200){
                $(tr + " td:nth-child(6)").html("审核不通过").append("<div class='passBox'>已驳回</div>");
