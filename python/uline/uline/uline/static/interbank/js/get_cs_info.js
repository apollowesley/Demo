...
function openNewTable(value,_status,type) {
    ...
    $.ajax({
        ...
        data: {"mch_id": value},
        dataType: 'json',
        url: '/inter_bank/inlet/cs/detail'
    }).done(function (res) {
        loadingHide();
        if (res.code == '200') {
            var data = res.data;
            ...
            if(data.balance_status != 1 ){
                roleHtml += '<div class="role-box col-md-12">' +
                            ...
                            '<a href="javascript:;" class="m-left-5" _params={"role_id":"'+value+'","role_type":"cs","platform":"1","action":"close"} data-click="settleOperate">冻结</a> ' +
                            '</div>';
            }else{
                roleHtml += '<div class="role-box col-md-12">' +
                            '<span class="m-right-5">结算账户状态：系统冻结</span>' +
                            '<a href="javascript:;" class="m-left-5" _params={"role_id":"'+value+'","role_type":"cs","platform":"","action":"open"} data-click="settleOperate">恢复</a> ' +
                            '</div>';
            }
            /*-------------------信用卡操作-----------------------*/
            if(data.credit_status){
                if(data.credit_status.weixin != 1 ){
                    roleHtml += '<div class="role-box col-md-12">' +
                                ...
                                '<a href="javascript:;" class="m-left-5" _params={"role_id":"'+value+'","role_type":"cs","platform":"weixin","action":"close"} data-click="creditOperate">关闭</a> ' +
                                '</div>';
                }else{
                    roleHtml += '<div class="role-box col-md-12">' +
                                '<span class="m-right-5">微信支付信用卡支付权限：已关闭</span>' +
                                '<a href="javascript:;" class="m-left-5" _params={"role_id":"'+value+'","role_type":"cs","platform":"weixin","action":"open"} data-click="creditOperate">开启</a> ' +
                                '</div>';
                }
                if(data.credit_status.alipay != 1 ){
                    roleHtml += '<div class="role-box col-md-12">' +
                                '<span class="m-right-5">支 付 宝 信用卡支付权限：正常</span>' +
                                '<a href="javascript:;" class="m-left-5" _params={"role_id":"'+value+'","role_type":"cs","platform":"alipay","action":"close"} data-click="creditOperate">关闭</a> ' +
                                '</div>';
                }else{
                    roleHtml += '<div class="role-box col-md-12">' +
                                '<span class="m-right-5">支 付 宝 信用卡支付权限：已关闭</span>' +
                                '<a href="javascript:;" class="m-left-5" _params={"role_id":"'+value+'","role_type":"cs","platform":"alipay","action":"open"} data-click="creditOperate">开启</a> ' +
                                '</div>';
                }
            }
...
function activatedMchInletStatus(mch_id, pay_type) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: '/inter_bank/inlet/cs/activated',
...
function activatedBatchMchInletStatus(mch_id) {
    ...
    $.ajax({
        ...
        data: {"mch_id": mch_id},
        dataType: 'json',
        url: '/inter_bank/inlet/cs/batchActivated',
        beforeSend: function (XMLHttpRequest) {
            // 请求前处理
        },
        success: function (res) {
