...
function uploadMchInfo() {
    ...
    excel.fileinput({
        language: 'zh', //设置语言
        uploadUrl: '/dist/inlet/cs/batch/excel',
...
function initUploadImg() {
    ...
    image.fileinput({
        language: 'zh', //设置语言
        uploadUrl: '/dist/inlet/cs/batch/image?uuid=' + uuid,
        ...
        if (data.response.code == 200) {
            if (uuid) {
                ...
                document.getElementById('batchMchInlet').action = '/dist/inlet/cs/batch/info?uuid=' + uuid;
                var goNext = $('#goNext');
                var buttonHtml = "";
