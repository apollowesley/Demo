        ...
        if (res.code == '200') {
            ...
            if (data.auth_status==2 || data.auth_status==3 || data.auth_status == 7 ){
                toUrl = '/chain/inlet/cs/modify?mch_id='+data.mch_id+'&imgCardFrontOld='+imgCardFrontOld
                    ...
                    +'&industry_no='+data.u_ind_code +'&email='+data.email+'&address='+data.address
                    ...
                    +'&balance_name='+data.balance_name+'&bank_no='+data.bank_no+'&bank_name='+data.bank_name
                    +'&balance_account='+data.balance_account+'&id_card_no='+data.id_card_no+'&industry_name='+data.industry_name
            ...
            appendHtml = appendHtml + "<li class='col-md-4 col-sm-4 leftLine' id='payType' onclick='openPayTab()'>支付信息</li>";
            ...
            if (data.status == 2) {
                ...
                authMchStatusHtml = authMchStatusHtml + "</div>";
                authMchStatusHtml = authMchStatusHtml + "<div>";
                // authMchStatusHtml = authMchStatusHtml + "<label>微信门店编号:</label>";
                //微信门店编号为空时
            ...
            appendMchInfoHtml = appendMchInfoHtml + "<div><label>门店简称</label><span>"+data.mch_shortname+"</span></div>";
...
function resendActiveEmail(mch_id){
    ...
    $.ajax({
        ...
        data: {"mch_id": mch_id},
        dataType: 'json',
        url: '/chain/inlet/cs/resend_active_email'
    }).done(function(res){
        var code = res.code;
