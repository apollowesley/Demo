    ...
    $.ajax({
        ...
        if (res.code == '200') {
            ...
            for (var i = 0; i < data.payment.length; i++) {
                for (_key in AVAILABLE_PAYMENTS) {
                    if (data.payment[i].uline_payment_code == _key) {
                        payHtml += '<div class="col-md-6 col-sm-6" style="text-align:left;padding-left:10px;">' + AVAILABLE_PAYMENTS[_key] + '</div><div class="col-md-2 col-sm-2">' + data.payment[i].settle_rate + '‰ </div>';
                        if (data.auth_status == 2 && (data.payment[i].activated_status == 1 || data.payment[i].activated_status == 3)) {
                            ...
                            if (data.bk_id == 1 && data.payment[i].operate == 1) {
                                activatedHtml = "<a href='javascript:;' id=" + "td_" + data.payment[i].pay_type + " onclick=activatedMchInletStatus(" + data.mch_id + ",'" + data.payment[i].uline_payment_code + "')>激活</a >"
                            } else {
                                activatedHtml = '';
                            }
            ...
            if (data.balance_status != 1) {
                ...
                if (data.bk_id == 1) {
                    roleHtml += '<a href="javascript:;" class="m-left-5" _params={"role_id":"' + value + '","role_type":"mch","platform":"1","action":"close"} data-click="settleOperate">冻结</a> ';
                }
                roleHtml += '</div>';
            } else {
                roleHtml += '<div class="role-box col-md-12">' +
                ...
                if (data.bk_id == 1) {
                    roleHtml += '<a href="javascript:;" class="m-left-5" _params={"role_id":"' + value + '","role_type":"mch","platform":"","action":"open"} data-click="settleOperate">恢复</a> ';
                }
                roleHtml += '</div>';
            }
            /*-------------------信用卡操作-----------------------*/
            if (data.credit_status) {
                if (data.credit_status.weixin != 1) {
                    ...
                    if (data.bk_id == 1) {
                        roleHtml += '<a href="javascript:;" class="m-left-5" _params={"role_id":"' + value + '","role_type":"mch","platform":"weixin","action":"close"} data-click="creditOperate">关闭</a> '
                    }
                    roleHtml += '</div>';
                } else {
                    roleHtml += '<div class="role-box col-md-12">' +
                    ...
                    if (data.bk_id == 1) {
                        roleHtml += '<a href="javascript:;" class="m-left-5" _params={"role_id":"' + value + '","role_type":"mch","platform":"weixin","action":"open"} data-click="creditOperate">开启</a> '
                    }
                    roleHtml += '</div>';
                }
                if (data.credit_status.alipay != 1) {
                    ...
                    if (data.bk_id == 1) {
                        roleHtml += '<a href="javascript:;" class="m-left-5" _params={"role_id":"' + value + '","role_type":"mch","platform":"alipay","action":"close"} data-click="creditOperate">关闭</a> '
                    }
                    roleHtml += '</div>';
                } else {
                    roleHtml += '<div class="role-box col-md-12">' +
                    ...
                    if (data.bk_id == 1) {
                        roleHtml += '<a href="javascript:;" class="m-left-5" _params={"role_id":"' + value + '","role_type":"mch","platform":"alipay","action":"open"} data-click="creditOperate">开启</a> '
                    }
                    roleHtml += '</div>';
                }
            }
            ...
            if(data.activated_status==1){
                $("#tr_"+value).find('td').eq(4).text('未激活');
            }else if(data.activated_status==2){
                $("#tr_"+value).find('td').eq(4).text('已激活');
            }
...
function activatedBatchMchInletStatus(mch_id) {
    ...
    $.ajax({
        ...
        data: {"mch_id": mch_id},
        dataType: 'json',
        url: '/bank/inlet/mch/batchActivated',
        beforeSend: function (XMLHttpRequest) {
            // 请求前处理
        },
        success: function (res) {
