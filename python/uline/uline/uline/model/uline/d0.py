...
from sqlalchemy import BigInteger, Column, DateTime, String, SmallInteger, Index, text, Date
...
class D0Withdraw(Base):
    ...
    fee_date = Column(Date)


class D0Repayment(Base):
    # d0 借款还款表
    __tablename__ = 'd0_repayment'

    id = Column(BigInteger, primary_key=True)
    # --借款ID  可重复
    loan_id = Column(BigInteger, nullable=False, default=0, server_default='0')
    # --还款ID
    repayment_id = Column(BigInteger, nullable=False, default=0, server_default='0')
    # --还款金额
    repayment_amount = Column(BigInteger, nullable=False, default=0, server_default='0')
    # --手续费
    draw_fee = Column(BigInteger, nullable=False, default=0, server_default='0')
    # --还款状态 0失败 1成功
    repayment_status = Column(SmallInteger, nullable=False, default=0, server_default='0')
    # --还款备注
    remark = Column(String(128), nullable=False, default="", server_default='')
