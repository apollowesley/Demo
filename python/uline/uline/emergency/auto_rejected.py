...
def get_dt_inlet_fail_mch(dt_id):
    with db.get_db() as cur:
        sql = """select mch_id from mch_inlet_info where dt_id=%s and auth_status='6';"""
...
def auth_mch_inlet(mch_id, cursor):
...
def add_auth_inlet_info(mch_id, cursor):
    comment = get_comment(mch_id, cursor)
    status = 3
    ...
    query = """
          ...
          values (%s, %s, %s, %s, %s);"""
    cursor.execute(query, (mch_id, comment, auth_user, status, create_at))


def get_comment(mch_id, cursor):
    query = """select comment from auth_mch_info where mch_id=%s ORDER BY create_at DESC LIMIT 1;"""
    cursor.execute(query, (mch_id,))
    ret = cursor.fetchone()
    return ret[0]
...
def main():
    ...
    if dt_id:
        with db.get_db() as cur:
            mch_ids = get_dt_inlet_fail_mch(dt_id)

            for mch_id in mch_ids:
                add_auth_inlet_info(mch_id, cur)
                auth_mch_inlet(mch_id, cur)

                message_id = gen_randown_mch_pkey(8)
                tasks.callback_for_merchant_status.apply_async((mch_id, 3, message_id), )
