...
from uline_risk.settings import ALI_APPID, ALI_PRIVATE_KEY, ALI_PUB_KEY, ALI_SERVER_ADDRESS
...
class AliEncryptionClass(object):
    ...
    def verify(data, sign, public_key_file, hash_type='RSA'):
        ...
        try:
            ...
            signature = verifier.verify(h, base64.decodebytes(sign))
...
def query_mapi_info(method, biz_content, config_info, crypto_type='RSA', charset='UTF-8'):
    ...
    sign_util = AliEncryptionClass(common_params, config_info, crypto_type, charset)
    sign = sign_util.sign()
    common_params['sign'] = sign
