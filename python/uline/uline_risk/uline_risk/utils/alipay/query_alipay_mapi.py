...
from uline_risk import settings
from uline_risk.settings import ALI_APPID, ALI_PID, ALI_PRIVATE_KEY, ALI_PUB_KEY
from uline_risk.utils import log
from uline_risk.utils.alipay.new_merchantInletToAlipay import query_mapi_info
...
def deal_excess_key(query_args, accept_keys):
    ...
    for single_key in query_args.keys():
        if single_key not in accept_keys:
            pop_keys.append(single_key)
    for each_key in pop_keys:
        query_args.pop(each_key)
    return query_args
